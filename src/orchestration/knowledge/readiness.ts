...
export function scoreTicketReadiness(ticket: GoalTicketPlan): TicketReadinessScore {
  ...
  const riskClarity = /risk|safe|fallback|error/i.test(ticket.description) ? 5 : 2;
  const fallbackBehavior = /fallback|missing knowledge|stale/i.test(ticket.description + ticket.acceptanceCriteria.join(" ")) ? 5 : 2;
  const nonGoalClarity = /do not|non-goal/i.test(ticket.description) ? 5 : 0;
